package com.analyzer.api.condition;

/**
 * 租房筛选条件实体类
 * @author 邢少
 *
 */
public class RentalCondition extends Condition{
	public RentalCondition(int maxPrices, int minPrices, String format, int maxArea, int minArea, String region, String detail, String other) {
		this.maxPrices = maxPrices;
		this.minPrices = minPrices;
		this.format = format;
		MaxArea = maxArea;
		MinArea = minArea;
		this.region = region;
		this.detail = detail;
		this.other = other;
	}
	//prices, format, area, location, other
	
	//最大价格
	private int maxPrices;
	//最小价格
	private int minPrices;
	//版式
	private String format;
	//面积
	private int MaxArea;

	private int MinArea;

		//区
	private String region;
		//详细
		private String detail;
	//其他，如靠近地铁等
	private String other;

	public String getRegion() {
		return region;
	}

	public void setRegion(String region) {
		this.region = region;
	}

	/**
	 * @return the maxPrices
	 */
	public int getMaxPrices() {
		return maxPrices;
	}
	/**
	 * @param maxPrices the maxPrices to set
	 */
	public void setMaxPrices(int maxPrices) {
		this.maxPrices = maxPrices;
	}
	/**
	 * @return the minPrices
	 */
	public int getMinPrices() {
		return minPrices;
	}
	/**
	 * @param minPrices the minPrices to set
	 */
	public void setMinPrices(int minPrices) {
		this.minPrices = minPrices;
	}
	/**
	 * @return the format
	 */
	public String getFormat() {
		return format;
	}
	/**
	 * @param format the format to set
	 */
	public void setFormat(String format) {
		this.format = format;
	}
	/**
	 * @return the province
	 */
	public String getDetail() {
		return detail;
	}
	/**
	 * @param detail the detail to set
	 */
	public void setDetail(String detail) {
		this.detail = detail;
	}
	/**
	 * @return the other
	 */
	public String getOther() {
		return other;
	}
	/**
	 * @param other the other to set
	 */
	public void setOther(String other) {
		this.other = other;
	}
	/* (non-Javadoc)
	 * @see java.lang.Object#toString()
	 */

	public int getMaxArea() {
		return MaxArea;
	}

	public void setMaxArea(int maxArea) {
		MaxArea = maxArea;
	}

	public int getMinArea() {
		return MinArea;
	}

	public void setMinArea(int minArea) {
		MinArea = minArea;
	}
}
